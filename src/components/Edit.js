    ...
    return (
        <>
            <div className="Edit flex justify-between">
                ...
                <button className={`text-white font-semibold px-5 py-1 text-lg bg-gray-700 rounded-full shadow-xl ${props.deleteMode ? 'bg-red-500' : 'bg-gray-700'}`} onClick={() => {
                    console.log('from edit', props.deleteMode); return props.toggleDeleteMode()}}>{props.deleteMode ? 'x' : '-'}</button>
            </div>
